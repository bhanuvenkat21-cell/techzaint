...
const ProgressBar = ({ value, max, label, inverse = false }: { value: number; max: number; label: string; inverse?: boolean }) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
          <span>Release Date: <strong className="text-gray-800">{productData.launchDate}</strong></span>
          <span className="flex items-center gap-1"><StarRating rating={productData.userRating} size={14} /><strong className="text-gray-800">{productData.userRating}/5</strong> ({fmt(productData.userRatingCount)} User Ratings)</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-2xl font-bold text-gray-900">₹{fmt(productData.price)}</p>
              <p className="text-xs text-gray-500">Available in 1 store</p>
            </div>
            <button className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold transition-colors">
              <ShoppingCart size={18} />See Prices
            </button>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {productData.awards.map((award, idx) => (
              <span key={idx} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg text-xs font-medium text-amber-700">
                <Award size={12} />{award}
              </span>
            ))}
          </div>
          ...
          <div className="flex gap-3">
            <button className="flex-1 flex items-center justify-center gap-2 bg-gray-900 hover:bg-black text-white py-3 rounded-xl font-semibold transition-colors"><GitCompare size={18} />Add To Compare</button>
          </div>
        </div>
      </div>
    </div>
