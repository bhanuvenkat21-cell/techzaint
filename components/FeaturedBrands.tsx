...
export default function FeaturedBrands() {
  ...
  const loopBrands = [...featuredBrands, ...featuredBrands];

  const BrandCard = ({ b }: { b: (typeof featuredBrands)[number] }) => (
    <Link
      href={`/mobiles/${b.slug}`}
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-6 relative overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold text-gray-900">Featured Mobile Brands</h2>
        <button
          type="button"
